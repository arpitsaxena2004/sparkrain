...
class RainwaterChatbot:
    def __init__(self):
        ...
        self.intent_patterns = {
            'definition': [
                'define', 'definition', 'what is', 'meaning', 'explain', 'describe',
                'what does', 'tell me about', 'what exactly is', 'concept of'
            ],
            'greeting': [
    ...
    def classify_intent(self, message: str) -> str:
        ...
        # Priority check for definition requests
        if any(word in message_lower for word in ['define', 'definition', 'what is', 'meaning', 'explain', 'describe']):
            return 'definition'
    ...
    def handle_definition(self, message: str) -> str:
        """Handle definition requests for rainwater harvesting"""
        message_lower = message.lower()
        
        if 'rainwater harvesting' in message_lower or 'rain water harvesting' in message_lower:
            return "🌧️ **Rainwater Harvesting Definition**:\n\nRainwater harvesting is the **collection, storage, and use of rainwater** from rooftops and other surfaces for various purposes.\n\n**Key Components**:\n• **Collection**: Gutters and pipes collect rainwater from roofs\n• **Storage**: Tanks store the collected water\n• **Treatment**: Filters clean the water for use\n• **Distribution**: Pumps and pipes deliver water when needed\n\n**Uses**: Drinking (after treatment), gardening, cleaning, toilet flushing, and groundwater recharge.\n\n**Benefits**: Reduces water bills, conserves groundwater, prevents flooding, and ensures water security during droughts."
        
        return "I can help define rainwater harvesting concepts! What specific aspect would you like me to explain?"
