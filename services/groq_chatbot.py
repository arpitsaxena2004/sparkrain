...
Answer all questions related to rainwater harvesting, water conservation, environmental sustainability, and related topics. If asked about unrelated topics, politely redirect to rainwater harvesting."""
    ...
    def get_groq_response(self, message: str, user_context: Dict = None) -> Optional[str]:
        ...
        try:
            ...
            # Use configuration from Django settings with fallbacks
            model = self.chatbot_config.get('GROQ_MODEL', 'llama-3.1-70b-versatile')
            ...
            if 'api key' in error_msg or 'unauthorized' in error_msg:
                print(f"❌ Groq API authentication error: {e}")
                print("   Please check your API key in settings.py")
            elif 'rate limit' in error_msg:
                print(f"⚠️  Groq API rate limit reached: {e}")
            elif 'network' in error_msg or 'connection' in error_msg:
                print(f"🌐 Groq API network error: {e}")
            else:
    ...
    def get_response(self, message: str, user_context: Dict = None) -> str:
        """Main method to get chatbot response with improved error handling"""
        if not message or not message.strip():
            return "I'm here to help with rainwater harvesting questions! What would you like to know?"
        
        # Check if fallback is enabled in configuration
        fallback_enabled = self.chatbot_config.get('FALLBACK_ENABLED', True)
        ...
        if self.client:
            try:
                ...
                print(f"Unexpected error in Groq response: {e}")
